...
public sealed class CliSubprocessTool : ITool
{
    ...
    public IList<string> ValidateParameters(Dictionary<string, object?>? parameters)
    {
        ...
        if (IsJsonMode)
        {
            ...
            {
                if (JsonSerializer.SerializeToElement(argsValue).ValueKind != JsonValueKind.Object)
                {
        ...
        ToolExecutionContext context)
    {
        ...
        if (errors.Count > 0) return ToolResult.Failure(string.Join(" ", errors));
        ...
        CancellationToken ct)
    {
        ...
        // Drain both pipes before writing stdin: a child may emit a full pipe
        // before reading its input, so sequential I/O can deadlock.
