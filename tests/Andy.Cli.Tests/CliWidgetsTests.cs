...
public class UserBubbleItemTests
{
    ...
    public void PromptLine_Uses_Light_Blue_Text_Color()
    {
        ...
        Assert.NotEqual(default(DL.TextRun), textWithContent);
    ...
    public void MarkdownRenderer_Uses_Colored_List_Markers()
    {
        ...
        Assert.NotEqual(default(DL.TextRun), bulletMarker);

        var numberMarker = textRuns.FirstOrDefault(tr => tr.Content == "1");
        Assert.NotEqual(default(DL.TextRun), numberMarker);
    }
    ...
    public void CodeBlockItem_Renders_With_Line_Numbers()
    {
        ...
        Assert.NotEqual(default(DL.TextRun), lineOne);

        // Should have line number "  2"
        var lineTwo = textRuns.FirstOrDefault(tr => tr.Content.Trim() == "2");
        Assert.NotEqual(default(DL.TextRun), lineTwo);
        ...
        Assert.NotEqual(default(DL.TextRun), codeContent);
    }
    ...
    [Fact]
    public void CodeBlockItem_Uses_Python_Syntax_Comments_Color_When_Language_Is_Python()
    {
        // Arrange
        var code = "print('hi')  # comment";
        var codeBlock = new CodeBlockItem(code, "python");
        var baseDl = new DL.DisplayListBuilder().Build();
        var builder = new DL.DisplayListBuilder();

        // Act
        codeBlock.RenderSlice(0, 0, 60, 0, 5, baseDl, builder);
        var dl = builder.Build();

        // Assert: ensure a run containing '# comment' exists and uses comment color (120,140,120)
        var textRuns = dl.Ops.OfType<DL.TextRun>().ToList();
        var commentRun = textRuns.FirstOrDefault(tr => tr.Content.Contains("# comment"));
        Assert.NotEqual(default(DL.TextRun), commentRun);
        Assert.Equal(120, commentRun.Fg.R);
        Assert.Equal(140, commentRun.Fg.G);
        Assert.Equal(120, commentRun.Fg.B);
    }

    [Fact]
    public void AddReplyToFeed_Extracts_Malformed_Single_Backtick_Language_Block()
    {
        // Arrange
        var feed = new FeedView();
        string reply = "Text before\n`csharp\nConsole.WriteLine(\"Hello\");\n`\nText after";

        var programType = typeof(Andy.Cli.Program);
        var addReply = programType.GetMethod("AddReplyToFeed", BindingFlags.NonPublic | BindingFlags.Static);
        Assert.NotNull(addReply);

        // Act
        addReply!.Invoke(null, new object[] { feed, reply, 0, 0 });

        // Assert via internal test hook: captured CodeBlockItem has language 'csharp'
        var items = feed.GetItemsForTesting();
        var codeItem = items.OfType<CodeBlockItem>().FirstOrDefault();
        Assert.NotNull(codeItem);
        Assert.Equal("csharp", codeItem!.GetLanguageForTesting());
    }

    [Fact]
    public void Inline_Tool_JSON_Is_Rendered_As_Json_Code_Block()
    {
        // Arrange
        var feed = new FeedView();
        string reply = "Before\n{\"tool\":\"write_file\",\"parameters\":{\"file_path\":\"Program.cs\",\"content\":\"class X{}\"}}\nAfter";
        ...
        // Assert via internal test hook: there is a json code block
        var items = feed.GetItemsForTesting();
        var jsonCodeItem = items.OfType<CodeBlockItem>().FirstOrDefault();
        Assert.NotNull(jsonCodeItem);
        Assert.Equal("json", jsonCodeItem!.GetLanguageForTesting());
    }
}