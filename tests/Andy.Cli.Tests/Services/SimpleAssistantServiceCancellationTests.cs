...
public class SimpleAssistantServiceCancellationTests
{
    ...
    {
        using var service = CreateService(new BlockUntilCancelledLlmProvider());
    ...
    public async Task ProcessMessageAsync_WhenTokenCancelledMidRun_ThrowsOperationCanceled()
    {
        ...
        using var service = CreateService(new BlockUntilCancelledLlmProvider(onEnter: cts.Cancel));
