...
// Serialized with the other tests that mutate the process-global ANDY_PERMISSION_MODE /
// permission-injection env vars, so its bypass setting cannot leak into permission tests
// running in parallel.
...
public class InteractiveAssistantReproTests
{
    ...
    public async Task FollowUpTurnAfterToolCall_DoesNotNullReference()
    {
        ...
        var originalPermissionMode = Environment.GetEnvironmentVariable("ANDY_PERMISSION_MODE");
        ...
        {
            Environment.SetEnvironmentVariable("ANDY_PERMISSION_MODE", originalPermissionMode);
        }
    }
